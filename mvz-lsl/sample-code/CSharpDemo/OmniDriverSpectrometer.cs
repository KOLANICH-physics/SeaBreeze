...
public
class OmniDriverSpectrometer: Spectrometer {
  private
	static OmniDriver.CCoWrapper wrapper = new OmniDriver.CCoWrapper();
  private
	Logger logger = null;
  private
	bool initialized = false;
  private
	int specIndex = 0;// for now, hardcode to first enumerated spectrometer
  private
	int pixels = 0;
  private
  ...
  public
	bool setIntegrationTimeMilliseconds(ulong ms) {
		if(!initialized)
			return false;
