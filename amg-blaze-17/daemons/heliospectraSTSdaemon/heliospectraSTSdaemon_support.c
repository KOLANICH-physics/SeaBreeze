...
void preparation_STS_serialNumber(int *status, char *errorMessage) {
	...
	if(*status == noError) {
		if(sbapi_get_serial_number_features(sbAPI.stsDeviceID, status, &serialNumberID, 1) >= 1) {
			...
			sbapi_get_serial_number(sbAPI.stsDeviceID, serialNumberID, status, sbAPI.stsSerialNumber, sizeof(sbAPI.stsSerialNumber));

			// returns the number of bytes in the serial number
			if(sbapi_get_serial_number(sbAPI.stsDeviceID, serialNumberID, status, sbAPI.stsSerialNumber, sizeof(sbAPI.stsSerialNumber)) <= 0)
...
void preparation_STS_calibrationTable(int *status, char *errorMessage) {
	if(*status == noError) {
		if(sbapi_get_irrad_cal_features(sbAPI.stsDeviceID, status, &(sbAPI.stsIrradCalID), 1) == 1) {
			if(sbapi_irrad_calibration_read(sbAPI.stsDeviceID,
				   sbAPI.stsIrradCalID,
				   status,
				   sbAPI.stsCalibrationArray,
				   STS_PIXEL_COUNT) == STS_PIXEL_COUNT) {
...
void preparation_STS_wavelengthTable(int *status, char *errorMessage) {
	if(*status == noError) {
		...
		if(sbapi_spectrometer_get_wavelengths(sbAPI.stsDeviceID, sbAPI.stsSpectrometerID, status, sbAPI.stsWavelengthArray, STS_PIXEL_COUNT) == STS_PIXEL_COUNT)
...
int TakeFormattedSampleSpectrumSTS() {
	...
	if(returnValue == noError)
		sbapi_spectrometer_get_formatted_spectrum(sbAPI.stsDeviceID, sbAPI.stsSpectrometerID, &returnValue, sbAPI.stsFormattedSpectrumArray, STS_PIXEL_COUNT);
...
int TakeRawSampleSpectrumSTS() {
	...
	//sbapi_shutter_set_shutter_open(sbAPI.stsDeviceID, sbAPI.stsShutterID, &returnValue, 1);
	sbapi_spectrometer_get_unformatted_spectrum(sbAPI.stsDeviceID, sbAPI.stsSpectrometerID, &returnValue, sbAPI.stsRawSpectrumArray, STS_PIXEL_COUNT * 2);
...
int TakeFormattedDarkSpectrumSTS() {
	...
	if(returnValue == noError)
		sbapi_spectrometer_get_formatted_spectrum(sbAPI.stsDeviceID, sbAPI.stsSpectrometerID, &returnValue, sbAPI.stsFormattedDarkSpectrumArray, STS_PIXEL_COUNT);
...
int TakeRawDarkSpectrumSTS() {
	...
	//sbapi_shutter_set_shutter_open(sbAPI.stsDeviceID, sbAPI.stsShutterID, &returnValue, 0);
	if(returnValue == noError)
		sbapi_spectrometer_get_unformatted_spectrum(sbAPI.stsDeviceID, sbAPI.stsSpectrometerID, &returnValue, sbAPI.stsRawDarkSpectrumArray, STS_PIXEL_COUNT * 2);
