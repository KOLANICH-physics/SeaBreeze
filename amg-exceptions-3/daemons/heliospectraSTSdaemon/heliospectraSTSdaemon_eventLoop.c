...
void SendWavebandList(int clientFileDescriptor, int *connectedToClient) {
	...
	if(theLastIndex >= 0) {
		for(i = 0; i <= theLastIndex; i++) {
			...
			if(aWaveband != NULL) {
				sprintf(dataBuffer, "%d\t%d\t%d\n", aWaveband->index, aWaveband->wavelengthLow, aWaveband->wavelengthHigh);
				SendMessage(clientFileDescriptor, connectedToClient, dataBuffer);
			} else {
	...
	if(theLastIndex >= 0) {
		for(i = 0; i <= theLastIndex; i++) {
			...
			if(aWaveband != NULL) {
				...
				sprintf(dataBuffer, "%d\t%d\t%d\t%f\tuW/cm2\n", aWaveband->index, aWaveband->wavelengthLow, aWaveband->wavelengthHigh, aWaveband->value);
				SendMessage(clientFileDescriptor, connectedToClient, dataBuffer);
			} else {
				SendBadCommandFormat(clientFileDescriptor, connectedToClient);
				notOK = TRUE;
	...
	if(theLastIndex >= 0) {
		for(i = 0; i <= theLastIndex; i++) {
			...
			if(aWaveband != NULL) {
				...
				sprintf(dataBuffer, "%d\t%d\t%d\t%f\tuW/cm2\t%f\tumol q/m2/s\n", aWaveband->index, aWaveband->wavelengthLow, aWaveband->wavelengthHigh, uw_cm2, umol_m2_s);
				SendMessage(clientFileDescriptor, connectedToClient, dataBuffer);
			} else {
				SendBadCommandFormat(clientFileDescriptor, connectedToClient);
				notOK = TRUE;
...
int EventLoop(volatile int *exitFlag, int afileDescriptorForTCP) {
	...
	while(!(*exitFlag)) {
		...
		if((serverFileDescriptor = accept(afileDescriptorForTCP, &clientAddress, &clientAddressLength)) >= 0) {
			...
			while((!(*exitFlag)) && connectionOpen) {
				...
				if(pollValue > 0) {
					...
					if(read(serverFileDescriptor, clientMessage, sizeof(clientMessage)) > 0) {
						parseClientMessage(clientMessage, serverFileDescriptor, exitFlag, &connectionOpen);
					} else
