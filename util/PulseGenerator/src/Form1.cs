...
partial class Form1: Form {
  ...
  private
	void checkBoxLoop_CheckedChanged(object sender, EventArgs e) {
	...
	int getPulseWidthFromView(int i) {
		if(dataGridViewPulses.RowCount == 0 || i + 1 > dataGridViewPulses.RowCount)
			return 0;
		if(dataGridViewPulses.ColumnCount == 0 || CHANNEL_WIDTH_COL + 1 > dataGridViewPulses.ColumnCount)
			return 0;
	...
	int getPulseOffsetFromView(int i) {
		if(dataGridViewPulses.RowCount == 0 || i + 1 > dataGridViewPulses.RowCount)
			return 0;
		if(dataGridViewPulses.ColumnCount == 0 || CHANNEL_OFFSET_COL + 1 > dataGridViewPulses.ColumnCount)
			return 0;
	...
	int getIntraPulseDelayFromView(int i) {
		if(dataGridViewPulses.RowCount == 0 || i + 1 > dataGridViewPulses.RowCount)
			return 0;
		if(dataGridViewPulses.ColumnCount == 0 || CHANNEL_INTRA_DELAY_COL + 1 > dataGridViewPulses.ColumnCount)
			return 0;
	...
	bool getPulseEnabledFromView(int i) {
		if(dataGridViewPulses.RowCount == 0 || i + 1 > dataGridViewPulses.RowCount)
			return false;
		if(dataGridViewPulses.ColumnCount == 0 || CHANNEL_ENABLE_COL + 1 > dataGridViewPulses.ColumnCount)
			return false;
  ...
  private
	void backgroundWorkerGraph_ProgressChanged(object sender, ProgressChangedEventArgs e) {
		logger.flush();
		updateGraphRunning();
	}
