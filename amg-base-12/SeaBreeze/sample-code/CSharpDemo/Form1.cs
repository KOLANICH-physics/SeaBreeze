...
partial class Form1: Form {
  ...
  private
	void buttonSaveDark_Click(object sender, EventArgs e) {
  ...
  private
	void checkBoxEnableIrradiance_Click(object sender, EventArgs e) {
		logger.display("Irradiance {0}", checkBoxEnableIrradiance.Checked ? "enabled" : "disabled");
		processSpectrum();
	}

  private
	void numericUpDownTriggerMode_ValueChanged(object sender, EventArgs e) {
  ...
  private
	void PersistenceUpDown_ValueChanged(object sender, EventArgs e) {
		persistenceWindow = (uint) PersistenceUpDown.Value;
	}

  private
	void IntensityUpDown_ValueChanged(object sender, EventArgs e) {
		intensityDelta = (uint) IntensityUpDown.Value;
	}

  private
	void numericUpDownScansToAverage_ValueChanged(object sender, EventArgs e) {
  ...
  private
	void DrawMousePosition(ref ChartGraphics chartGraphics, Brush theBrush, float radius) {
		if(scanning) {
			Series spectrum = chartSpectrum.Series.FindByName("seriesSpectrum");
  ...
  private
	void chartSpectrum_onMouseMove(object sender, MouseEventArgs e) {
		if(scanning) {
			ChartArea chartArea = chartSpectrum.ChartAreas[0];
			try {
  ...
  private
	void chartSpectrum_onMouseLeave(object sender, EventArgs e) {
  ...
  private
	void updateGUIFromSpectrometer() {
  ...
  private
	void updateSpectrometerFromGUI() {
	...
	//
  private
	double[] computeAbsoluteIrradiance() {
		...
		// compute irradiance
		for(int i = 0; i < pixels; i++)
			irradiance[i] = (procSpectrum[i] * irradCal[i]) / (integrationTimeMillisec * 1000.0 * collectionArea * dLp[i]);
  ...
  private
	void backgroundWorkerAcquisition_DoWork(object sender, DoWorkEventArgs e) {
  ...
  private
	void backgroundWorkerAcquisition_ProgressChanged(object sender, ProgressChangedEventArgs e) {
		logger.flush();
		processSpectrum();
	}

  private
	void backgroundWorkerAcquisition_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e) {
  ...
  private
	void PeakFindCheckBox_CheckedChanged(object sender, EventArgs e) {
  ...
  private
	int FindPeaks(ref List<int> peakIndexes) {
...
public class TooManyPeaksFoundException: Exception {
  public
	TooManyPeaksFoundException() {
	}

  public
	TooManyPeaksFoundException(string message)
		: base(message) {
	}

  public
	TooManyPeaksFoundException(string message, Exception inner)
		: base(message, inner) {
	}
}
}// namespace CSharpDemo
