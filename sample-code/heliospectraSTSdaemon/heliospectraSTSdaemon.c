			...
			if((returnValue = getaddrinfo(NULL, tcpPortNumber, &networkHints, &tcpSocketInfoPointer)) == noError) {
				...
				for(addrInfoPtr = tcpSocketInfoPointer; addrInfoPtr != NULL;
					...
					if((fileDescriptorForTCP = socket(addrInfoPtr->ai_family,
							addrInfoPtr->ai_socktype,
							addrInfoPtr->ai_protocol)) >= 0)
						break;
					else {
				...
				if(addrInfoPtr != NULL) {
					...
					if(setsockopt(fileDescriptorForTCP, SOL_SOCKET, SO_REUSEADDR, (char *) &opt, sizeof(opt)) == noError) {
						// bind to the socket
						if(bind(fileDescriptorForTCP, addrInfoPtr->ai_addr, addrInfoPtr->ai_addrlen) == noError) {
